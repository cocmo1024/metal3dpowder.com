# Metal 3D Powder SEO Roadmap

## Working rule

Before publishing a new article:

1. Read the existing titles in `src/content/blog/`.
2. Check whether the search intent already exists as:
   - a material profile,
   - a comparison page,
   - a buying guide,
   - a pricing page,
   - or a quality/process guide.
3. If the intent already exists, update that URL instead of creating a near-duplicate page.
4. Use site pricing from `src/data/products.ts` when mentioning on-site indicative prices.
5. Avoid precise third-party price claims unless they can be verified from live public supplier pages.

## Existing inventory before round 1 expansion

### Material profiles already live

- TC4 / Ti-6Al-4V
- TA1 / CP titanium
- TA15
- 316L
- 17-4PH
- IN718
- IN625
- AlSi10Mg
- CuCrZr
- CoCrMo / CoCrW

### Gaps identified

- No dedicated content for H13, M300, or Hastelloy X even though the products are on the home page.
- No comparison articles for common buyer decisions.
- No RFQ, pricing, PSD, or certificate/COA guides.
- Several material pages were thin and needed stronger commercial context.

## Round 1 publishing map

### Commercial-intent guides

- Metal AM powder price guide
- Metal powder RFQ checklist
- LPBF powder particle size guide
- Metal powder COA and certificate guide

### Comparison pages

- Ti64 vs Grade 2 titanium powder
- IN718 vs IN625 powder
- 316L vs 17-4PH powder

### Missing product support pages

- H13 tool steel powder
- M300 maraging steel powder
- Hastelloy X powder

### Existing pages to upgrade in round 1

- TC4 / Ti-6Al-4V
- TA1 / CP titanium
- TA15
- 316L
- 17-4PH
- IN718
- IN625
- AlSi10Mg
- CuCrZr
- CoCrMo / CoCrW

## Round 2 backlog

- H13 vs M300
- CuCrZr vs pure copper powder for AM
- EBM powder size guide
- Powder reuse policy guide
- Export document checklist for powder shipments
- Medical and dental powder documentation guide
- Aerospace powder qualification checklist

## Round 2 publishing map

### Tooling and material-choice pages

- H13 vs M300 powder for AM tooling
- CuCrZr vs pure copper powder for AM

### Process and quality pages

- EBM powder size guide
- Metal powder reuse policy guide

### Commercial and compliance pages

- Export document checklist for international powder shipments
- Medical and dental powder documentation guide
- Aerospace powder qualification checklist

## Round 3 backlog

- Powder storage and shelf-life guide
- Prototype vs production powder specification guide
- Incoterms guide for powder buyers
- How to compare two powder quotes without false savings
- Titanium Grade 23 vs Grade 5 buying guide
- Dental lab cobalt chrome sourcing checklist

## Round 3 publishing map

### Commercial decision pages

- How to compare two powder quotes without false savings
- Prototype vs production powder specification guide
- Incoterms guide for international powder buyers

### Quality and handling pages

- Metal powder storage and shelf-life guide
- Titanium Grade 23 vs Grade 5 powder
- Dental lab cobalt chrome sourcing checklist

## Round 4 backlog

- Incoming inspection checklist for AM powder lots
- First article sample request guide for new suppliers
- Prototype-to-qualification document handoff guide
- MOQ and packaging strategy for international powder orders
- DED powder vs wire decision guide
- Cobalt chrome vs titanium for dental AM buying
